...
function Sender (params) {
  ...
  this.destination_ledger = params.destination_ledger || parseLedger(this.destination_account)
  this.destination_username = params.destination_username
  this.destination_account = params.destination_account
...
Sender.prototype.findPath = function * () {
  ...
  if (!this.subpayments || this.subpayments.length === 0) {
...
Sender.prototype.setupTransfers = function * () {
  ...
  // Add memo to destination transfer
...
function parseLedger (uri) {
  return uri.replace(new RegExp('\/accounts/.*$'), '')
}
